...
void free_req_msg(struct request_msg *r){
    if(r->func.s) pkg_free(r->func.s);r->func.s=NULL;r->func.len=0;
    if(r->devid.s) pkg_free(r->devid.s);r->devid.s=NULL;r->devid.len=0;
    if(r->phoneid.s) pkg_free(r->phoneid.s);r->phoneid.s=NULL;r->phoneid.len=0;
    if(r->snstype.s) pkg_free(r->snstype.s);r->snstype.s=NULL;r->snstype.len=0;
    if(r->snsname.s) pkg_free(r->snsname.s);r->snsname.s=NULL;r->snsname.len=0;
}
...
int parse_req_msg(char *b, struct request_msg *r){
    ...
    if(p2){
        ...
        while(isspace(*p3)) p3--; /* strip the space at end */
        p3++;
        ...
        while(isspace(*p2)) p2++; /* strip the space at start */
